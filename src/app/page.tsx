...
import { ArrowDown, ArrowUp, Plus, RefreshCw, Send, TrendingUp, Wallet, DollarSign } from "lucide-react"
...
export default function OverviewPage() {
  ...
  const activeWallets = [
    ...
    { name: "Ethereum Wallet", asset: "ETH", balance: 15.5, network: "Ethereum" },
  ...
  return (
    <div className="content-height p-8 scroll-area bg-background">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="glass-card-enhanced glass-hover">
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm muted-text font-medium">Total Portfolio</CardTitle>
            ...
            <CardContent>
              <div className="text-3xl font-bold primary-text">${balances.total.toLocaleString()}</div>
              <div className="flex items-center gap-2 mt-3">
                <div className="w-6 h-6 rounded-full bg-green-500/20 flex items-center justify-center">
                  <TrendingUp className="w-4 h-4 text-green-500" />
                </div>
                <span className="text-sm text-green-500 font-medium">+12.5% this month</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="glass-card-enhanced glass-hover">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-main">${balances.crypto.toLocaleString()}</div>
          ...
          <Card className="glass-card-enhanced glass-hover">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-main">${balances.fiat.toLocaleString()}</div>
              <div className="text-sm muted-text mt-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <Card className="glass-card-enhanced">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-xl font-bold text-main">Recent Transactions</CardTitle>
            ...
            <CardContent>
              <Tabs defaultValue="all" className="w-full">
                ...
                <TabsContent value="all" className="space-y-4 max-h-80 scroll-area">
                  {recentTransactions.map((tx) => (
                    <div key={tx.id} className="glass-item p-4 flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="font-bold text-main">${tx.value.toLocaleString()}</div>
                        <Badge
          ...
          <Card className="glass-card-enhanced">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-xl font-bold text-main">Active Wallets</CardTitle>
                <Button variant="ghost" size="sm" className="glass-button">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-primary text-white flex items-center justify-center shadow-md">
                        <span className="font-bold text-sm">{wallet.asset}</span>
                      </div>
                      <div>
                        <div className="font-semibold text-main">{wallet.name}</div>
                        <div className="text-sm muted-text">{wallet.network}</div>
                      </div>
                    </div>
      ...
      {/* Action Modals */}
      <ActionModal isOpen={activeModal === "deposit"} onClose={() => setActiveModal(null)} type="deposit" />
      <ActionModal isOpen={activeModal === "exchange"} onClose={() => setActiveModal(null)} type="exchange" />
      <ActionModal isOpen={activeModal === "send"} onClose={() => setActiveModal(null)} type="send" />
    </div>
  )
}
