    ...
    <div className="content-height p-8 scroll-area bg-background">
      <div className="max-w-7xl mx-auto space-y-8">
              ...
              <CardContent className="p-0">
                ...
                <div className={`relative h-56 bg-linear-to-br ${card.gradient} p-6 overflow-hidden`}>
                  ...
                  <div className={`relative z-10 h-full flex flex-col justify-between ${card.textColor}`}>
                    ...
                    <div className="space-y-3">
                      <div className="text-2xl font-mono tracking-wider font-bold">{card.number}</div>
                      <div className="flex justify-between items-end">
                        ...
                        <div className="text-right">
                          ...
                          <div className="flex items-center gap-1 mt-1">
                            ...
                            <span className="text-xs opacity-80">{card.status}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-main">{card.network}</span>
                  </div>
        ...
        <Card className="glass-card-enhanced">
          <CardHeader>
            <CardTitle className="text-xl font-bold text-main">OND4 Card Benefits</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="text-center space-y-4">
                ...
                <h3 className="font-bold text-lg text-main">Instant Spending</h3>
                <p className="text-sm muted-text">
                  Use your crypto balance instantly at millions of locations worldwide
                </p>
              </div>
              <div className="text-center space-y-4">
                ...
                <p className="text-sm muted-text">Spend abroad without worrying about foreign transaction fees</p>
              </div>
              <div className="text-center space-y-4">
                ...
                <p className="text-sm muted-text">Earn up to 5% back in crypto on all your purchases</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Dialog open={!!selectedCard} onOpenChange={() => setSelectedCard(null)}>
          <DialogContent className="glass-card-enhanced max-w-md">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-main">Card Details</DialogTitle>
            </DialogHeader>
            {selectedCard && (
              <div className="space-y-6">
                ...
                >
                  ...
                  <div className={`relative z-10 h-full flex flex-col justify-between ${selectedCard.textColor}`}>
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-bold text-lg">{selectedCard.name}</h3>
                        <p className="text-sm opacity-90">{selectedCard.type}</p>
                      </div>
                      <CreditCard className="w-8 h-8 opacity-80" />
                    </div>

                    <div className="space-y-2">
                      <div className="text-xl font-mono tracking-wider font-bold">{selectedCard.number}</div>
                      <div className="flex justify-between items-end">
                        <div>
                          <p className="text-xs opacity-80">BALANCE</p>
                        ...
                        <div className="text-right">
                          <p className="text-sm font-bold">{selectedCard.network}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="glass-item p-4">
                      ...
                      <p className="font-semibold text-main">{selectedCard.status}</p>
                    </div>
                    <div className="glass-item p-4">
                      <Label className="muted-text text-xs">Type</Label>
                      <p className="font-semibold text-main">{selectedCard.type}</p>
                    </div>
                  </div>

                  <div className="glass-item p-4">
                    <Label className="muted-text text-xs">Available Balance</Label>
                    <p className="text-2xl font-bold primary-text">${selectedCard.balance.toLocaleString()}</p>
                  </div>
                </div>

                {/* Card Actions */}
                <div className="grid grid-cols-2 gap-3">
                  <Button className="glass-button bg-transparent" variant="outline">
                    View Transactions
                  </Button>
                  <Button className="glass-button bg-transparent" variant="outline">
                    Card Settings
                  ...
                  <Button className="bg-primary hover:bg-primary/90 text-white font-semibold">Top Up</Button>
                </div>
              </div>
            )}
          </DialogContent>
