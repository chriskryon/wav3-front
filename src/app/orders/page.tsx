...
export default function OrdersPage() {
  ...
  const filteredOrders =
    statusFilter === "all" ? orders : orders.filter((order) => order.status.toLowerCase() === statusFilter)
      ...
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm muted-text font-medium">Filter by status:</span>
              </div>
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-main font-semibold">{order.asset}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="muted-text text-sm">Quantity:</span>
                    <span className="text-main font-semibold">{order.amount}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="primary-text font-bold">${order.total.toLocaleString()}</span>
                  </div>
                </div>

                {/* Status and Date */}
                <div className="flex items-center justify-between pt-3 border-t border-black/10">
                  <Badge className={`text-xs font-medium ${getStatusColor(order.status)}`}>{order.status}</Badge>
        ...
        <Dialog open={showNewOrderModal} onOpenChange={setShowNewOrderModal}>
          <DialogContent className="glass-card-enhanced max-w-md">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-main">Create New Order</DialogTitle>
            </DialogHeader>
            <form className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium muted-text">Order Type</Label>
                  <Select>
              ...
              <div>
                <Label className="text-sm font-medium muted-text">Price (USD)</Label>
                <Input placeholder="0.00" className="glass-input mt-1" />
              </div>
        ...
        <Dialog open={!!selectedOrder} onOpenChange={() => setSelectedOrder(null)}>
          <DialogContent className="glass-card-enhanced max-w-md">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-main">Order Details</DialogTitle>
            </DialogHeader>
            {selectedOrder && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-semibold text-main">{selectedOrder.id}</p>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-semibold text-main">{selectedOrder.type}</p>
                  </div>
                  <div>
                    <Label className="muted-text text-sm">Asset</Label>
                    <p className="font-semibold text-main">{selectedOrder.asset}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-main">${selectedOrder.price.toLocaleString()}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="muted-text text-sm">Total</Label>
                    <p className="font-bold primary-text">${selectedOrder.total.toLocaleString()}</p>
                  </div>
                  <div>
                    <Label className="muted-text text-sm">Fee</Label>
                    <p className="font-semibold text-main">${selectedOrder.fee}</p>
                  </div>
                </div>
                <div>
                  <Label className="muted-text text-sm">Date</Label>
                  <p className="font-semibold text-main">{selectedOrder.date}</p>
                </div>
              </div>
            )}
          </DialogContent>
