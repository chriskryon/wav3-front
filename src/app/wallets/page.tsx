      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="muted-text text-lg">Manage your cryptocurrency wallets</p>
          </div>
          <Button
                  ...
                  <div className="relative z-10 h-full flex flex-col justify-between text-white">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-bold text-xl">${wallet.value.toLocaleString()}</span>
                      </div>
                    </div>

                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-mono opacity-80">{truncateAddress(wallet.address)}</span>
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={(e) => {
        ...
        <Dialog open={showNewWalletModal} onOpenChange={setShowNewWalletModal}>
          <DialogContent className="glass-card-enhanced max-w-md">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-main">Add New Wallet</DialogTitle>
            </DialogHeader>
            <form className="space-y-6">
              <div>
                <Label className="text-sm font-medium muted-text">Wallet Name</Label>
                <Input placeholder="My Bitcoin Wallet" className="glass-input mt-1" />
              </div>
              ...
              <div>
                ...
                <Input placeholder="Enter wallet address" className="glass-input mt-1" />
              </div>
        ...
        <Dialog open={!!selectedWallet} onOpenChange={() => setSelectedWallet(null)}>
          <DialogContent className="glass-card-enhanced max-w-md">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-main">Wallet Details</DialogTitle>
            </DialogHeader>
            {selectedWallet && (
              <div className="space-y-6">
                ...
                >
                  ...
                  <div className="relative z-10 h-full flex flex-col justify-between text-white">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-bold text-lg">{selectedWallet.name}</h3>
                        <p className="text-sm opacity-90">{selectedWallet.network} Network</p>
                      </div>
                      <Wallet className="w-8 h-8 opacity-80" />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm opacity-80">USD Value</span>
                        <span className="font-bold text-xl">${selectedWallet.value.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Button className="bg-primary hover:bg-primary/90 text-white font-semibold">Settings</Button>
                </div>
              </div>
            )}
          </DialogContent>
