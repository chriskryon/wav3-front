...
export function AppSidebar() {
  ...
  // Show My Profile as active if profile is not completed
  const shouldShowProfile = userData && !userData.profileCompleted
  const activeItems = shouldShowProfile
    ? [{ title: "My Profile", url: "/profile", icon: User }, ...menuItems.slice(1)]
    : menuItems
          ...
          <div className="backdrop-blur-sm bg-black/5 p-4 rounded-lg border border-black/10 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-main truncate">{userData.name || userData.email}</div>
